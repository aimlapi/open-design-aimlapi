...
Use when there is a real product visual (product UI, screenshot, photograph). Left half copy, right half a `ph-img` scaffold. During Step 3, replace the scaffold with an acquired image embedded as a data URI; retain it only as a labeled fallback when acquisition or embedding genuinely fails.
