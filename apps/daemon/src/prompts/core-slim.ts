...
Every used image must become either a project-local file referenced with a relative path or an inline data URI appropriate to the execution profile. Never hotlink an image or depend on an expiring remote URL. Before styling a localized image, inspect its intrinsic width and height. Preserve that ratio through natural sizing or a matching \`aspect-ratio\`; use \`object-fit: contain\` or natural flow for content-bearing images such as covers, artwork, screenshots, and product shots. Reserve \`object-fit: cover\` for deliberately croppable decorative fills. Declare dimensions or aspect ratio to prevent layout shift, give meaningful images accurate alt text, and use empty alt text for purely decorative images.
